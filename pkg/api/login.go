...
// SPHINX SM Login
func (hs *HTTPServer) LoginPostSM(c *models.ReqContext, loginModel dtos.LoginModel) Response {
	action := "login"
	// Decrypt credentials

	decryptedCredentials, err_ := Decrypt(loginModel.Sphinxsmticketaux, "7061737323313233")
	if err_ != nil {
		fmt.Println(err_)
	}
	...
	var cmd dtos.LoginCommand
	cmd.User = credentials[0]
	cmd.Password = credentials[1]
